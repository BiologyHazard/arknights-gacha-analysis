...
/// 水位三元组 → 压缩索引的查找表，仅对 五星水位 <= 六星水位 <= UP六星水位 的有效状态赋值
static 水位压缩查找表: OnceLock<(Vec<u32>, u32)> = OnceLock::new();

/// 初始化/获取水位压缩查找表，返回 (查找表, 有效水位三元组数量)
fn 获取水位映射() -> &'static (Vec<u32>, u32) {
    水位压缩查找表.get_or_init(|| {
        let 水位三元组总数 = (六星水位上限 as usize + 1)
            * (五星水位上限 as usize + 1)
            * (UP六星水位上限 as usize + 1);
        let mut 水位压缩编号表 = vec![u32::MAX; 水位三元组总数];
        let mut 有效水位三元组数 = 0u32;

        for 六星水位 in 0..=六星水位上限 {
            for 五星水位 in 0..=六星水位.min(五星水位上限) {
                for UP六星水位 in 六星水位..=UP六星水位上限 {
                    let 水位三元组扁平索引 = (六星水位 as usize * (五星水位上限 as usize + 1)
                        + 五星水位 as usize)
                        * (UP六星水位上限 as usize + 1)
                        + UP六星水位 as usize;
                    水位压缩编号表[水位三元组扁平索引] = 有效水位三元组数;
                    有效水位三元组数 += 1;
                }
            }
        }
...
/// 获取压缩后的状态总数 = 有效水位三元组数 × 7 × 7
pub fn 获取状态数量() -> u32 {
    let (_, 有效水位三元组数) = 获取水位映射();
    有效水位三元组数 * (已获取UP六星干员数量上限 as u32 + 1) * (已获取UP五星干员数量上限 as u32 + 1)
}
...
pub fn 获取状态索引(
    ...
    let (水位压缩编号表, _) = 获取水位映射();
    let 水位三元组扁平索引 = (六星水位 as usize * (五星水位上限 as usize + 1) + 五星水位 as usize)
        * (UP六星水位上限 as usize + 1)
        + UP六星水位 as usize;
    let 水位压缩编号 = 水位压缩编号表[水位三元组扁平索引];
    debug_assert!(
        水位压缩编号 != u32::MAX,
        "无效状态 ({六星水位}, {五星水位}, {UP六星水位})"
    );
    (水位压缩编号 * (已获取UP六星干员数量上限 as u32 + 1)
...
pub fn 状态转移(
    ...
    if 起始UP六星水位 == 119 {
        ...
        // 只有在概率 > 0 时才调用 获取状态索引，避免无效水位（如六星=99）越界
        let 六星半概率 = 六星概率 / 2.0;
        if 六星半概率 > 0.0 {
            转移概率列表.push((
                获取状态索引(
...
pub fn 构造状态转移矩阵(矩阵类型: 矩阵类型枚举) -> CooArray<f64, u32, u32> {
    ...
    for 起始六星水位 in 0..=六星水位上限 {
        for 起始五星水位 in 0..=起始六星水位.min(五星水位上限) {
            for 起始UP六星水位 in 起始六星水位..=UP六星水位上限 {
