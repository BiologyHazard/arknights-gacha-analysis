...
def draw_pmf_cdf_fig(dist: FiniteDist,
    ...
    ax1.plot(x, dist.pk, drawstyle=drawstyle, label="概率质量函数", path_effects=stroke_white, zorder=10)
    ax2.fill_between(x, dist.cdf(x), step="mid", alpha=0.3, zorder=3)
    ax2.plot(x, dist.cdf(x), drawstyle=drawstyle, label="累积分布函数", path_effects=stroke_white, zorder=10)

    # 期望值
    ax1.axvline(dist.expect(), color="C1", linestyle="--", label="期望值")
    ax1.annotate(f"期望\n{dist.expect():.2f}抽", (dist.expect(), ax1_y_top),
                 ha="center", va="top", xytext=(0, -5), textcoords="offset points",
                 color="C1", fontweight="medium", path_effects=stroke_white)
    ...
    [
        ax1.annotate(f"{quantile_point}抽\n{dist.cdf(quantile_point):.0%}", (quantile_point, ax1_y_top),
                     ha="center", va="top", xytext=(0, -5), textcoords="offset points",
                     color="gray", fontweight="medium", path_effects=stroke_white)
        for quantile_point in quantile_points
    ]
    ...
    # 期望值
    ax2.axvline(dist.expect(), color="C1", linestyle="--", label="期望值")
...
def draw_multi_cdf_fig(dists: list[FiniteDist],
    ...
    for dist, label, color in zip(dists, labels, colors):
        x = np.arange(len(dist.pk))
        ax.plot(x, dist.cdf(x), color=color, drawstyle=drawstyle, path_effects=stroke_white, label=label, zorder=10)

    # 期望值
    for dist, color in zip(dists, colors):
        ax.axvline(dist.expect(), color=color, linestyle="--")
        ax.annotate(f"期望\n{dist.expect():.2f}", (dist.expect(), ax_y_top),
                    ha="center", va="top", xytext=(0, -5), textcoords="offset points",
                    color=color, fontweight="medium", path_effects=stroke_white)
    ...
    for i, dist in enumerate(dists):
        ...
        [
            ax.annotate(quantile_point, (quantile_point, quantile_pos),
                        ...
                        color="gray", fontweight="medium", path_effects=stroke_white, zorder=20)
