...
# 网格线1 < 水印2 < 填充3 < 横线4 < 竖线5 < 曲线6 < 标记点7 < 右侧文字8 < 分位数文字9 < 期望文字10 < 众数文字11
...
def draw_pmf_cdf_fig(dist: FiniteDist,
    ...
    quantile_points = calc_quantile_point(dist.cdf(x), quantile_poses)
    ...
    ax1.fill_between(x, dist.pk, step="mid", alpha=0.3, zorder=3)
    ax1.plot(x, dist.pk, drawstyle=drawstyle, path_effects=stroke_white, zorder=6)
    ax2.fill_between(x, dist.cdf(x), step="mid", alpha=0.3, zorder=3)
    ax2.plot(x, dist.cdf(x), drawstyle=drawstyle, path_effects=stroke_white, zorder=6)

    # 期望值
    ax1.axvline(dist.expect(), linestyle="--", color="C1", zorder=5)
    ax1.annotate(f"期望\n{dist.expect():.2f}抽", (dist.expect(), ax1_y_top),
                 ha="center", va="top", xytext=(0, -5), textcoords="offset points",
                 color="C1", fontweight="medium", path_effects=stroke_white, zorder=10)

    # 分位数
    [
        ax1.axvline(quantile_point, color="gray", linestyle="--", zorder=6)
        for quantile_point in quantile_points
    ]
    [
        ax1.annotate(f"{quantile_point}抽\n{dist.cdf(quantile_point):.0%}", (quantile_point, ax1_y_top),
                     ha="center", va="top", xytext=(0, -5), textcoords="offset points",
                     color="gray", fontweight="medium", path_effects=stroke_white, zorder=9)
        for quantile_point in quantile_points
    ]

    # 众数
    ax1.scatter(np.argmax(dist.pk), np.max(dist.pk),
                color="C1", marker=".", path_effects=stroke_white, zorder=7)
    ax1.annotate(f"最有可能\n{np.argmax(dist.pk)}抽", (np.argmax(dist.pk), np.max(dist.pk)),  # type: ignore
                 ha="center", va="bottom", xytext=(0, 5), textcoords="offset points",
                 color="C1", fontweight="medium", path_effects=stroke_white, zorder=11)

    # 期望值
    ax2.axvline(dist.expect(), linestyle="--", color="C1", zorder=5)
    ax2.annotate(f"期望\n{dist.expect():.2f}抽", (dist.expect(), ax2_y_top),
                 ha="center", va="top", xytext=(0, -5), textcoords="offset points",
                 color="C1", fontweight="medium", path_effects=stroke_white, zorder=10)

    # 分位数
    ax2.scatter(quantile_points, dist.cdf(quantile_points),
                color="C0", marker=".", path_effects=stroke_white, zorder=7)
    [
    ...
    for ax in (ax1, ax2):
        ...
        ax.text(0.98, 0.02, "bilibili@Bio-Hazard", transform=ax.transAxes, ha="right", va="bottom",
                fontsize=14, fontweight="medium", color="gray", path_effects=stroke_white, zorder=2)
...
def draw_multi_cdf_fig(dists: list[FiniteDist],
                       labels: list[str],
                       title: str,
                       quantile_poses,
                       x_max: float,
    ...
    for dist, label, color in zip(dists, labels, colors):
        x = np.arange(len(dist.pk))
        ax.plot(x, dist.cdf(x), color=color, drawstyle=drawstyle, path_effects=stroke_white, label=label, zorder=6)

        # 期望值
        ax.axvline(dist.expect(), linestyle="--", color=color, zorder=5)
        ax.annotate(f"期望\n{dist.expect():.2f}", (dist.expect(), ax_y_top),
                    ha="center", va="top", xytext=(0, -5), textcoords="offset points",
        ...
        ax.scatter(quantile_points, dist.cdf(quantile_points),
                   s=10, color=color, marker=".", path_effects=stroke_white, zorder=7)
        [
...
def draw_multi_pmf_cdf_fig(dists: list[FiniteDist],
                           labels: list[str],
                           title: str,
                           quantile_poses,
                           *,
    ...
    if ax1_y_top is None:
        ax1_y_top = np.max(dists[0].pk) * 1.27
    ax2_y_top = 1.2
    ...
    for dist, label, color in zip(dists, labels, colors):
        ...
        ax1.plot(x, dist.pk, label=label,
                 color=color, drawstyle=drawstyle, path_effects=stroke_white, zorder=6)
        # ax2.fill_between(x, dist.cdf(x), step="mid", alpha=0.3, zorder=3)
        ax2.plot(x, dist.cdf(x), label=label,
                 color=color, drawstyle=drawstyle, path_effects=stroke_white, zorder=6)

        # 期望值
        ax1.axvline(dist.expect(), linestyle="--", color=color, zorder=5)  # type: ignore
        ax1.annotate(f"期望\n{dist.expect():.2f}", (dist.expect(), ax1_y_top),  # type: ignore
                     ha="center", va="top", xytext=(0, -5), textcoords="offset points",
                     color=color, fontweight="medium", path_effects=stroke_white, zorder=10)
        ax2.axvline(dist.expect(), linestyle="--", color=color, zorder=5)  # type: ignore
        ax2.annotate(f"期望\n{dist.expect():.2f}", (dist.expect(), ax2_y_top),  # type: ignore
                     ha="center", va="top", xytext=(0, -5), textcoords="offset points",
                     color=color, fontweight="medium", path_effects=stroke_white, zorder=10)
        ...
        ax1.scatter(np.argmax(dist.pk), np.max(dist.pk),
                    s=10, color=color, marker=".", path_effects=stroke_white, zorder=7)
        ax1.annotate(f"最有可能\n{np.argmax(dist.pk)}", (np.argmax(dist.pk), np.max(dist.pk)),  # type: ignore
                     ha="center", va="bottom", xytext=(0, 5), textcoords="offset points",
                     color=color, fontweight="medium", path_effects=stroke_white, zorder=11)
        ...
        [
            ax2.axhline(quantile_pos, color="gray", linestyle="--", zorder=4)
            for quantile_pos in quantile_poses
        ]
        [
